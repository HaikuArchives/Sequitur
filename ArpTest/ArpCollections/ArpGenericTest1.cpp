		  ...
		  :BApplication(app_signature)
{
	...
	printf("*********************TestNodeAt_Forwards02\n");  fflush(stdout);
	TestNodeAt_Forwards02();
	printf("*********************TestNodeFor01\n");  fflush(stdout);
	TestNodeFor01();
	printf("*********************TestCount01\n");  fflush(stdout);
	TestCount01();
	printf("*********************TestRemoveNode01\n");  fflush(stdout);
	TestRemoveNode01();
	printf("*********************TestTailNode02\n");  fflush(stdout);
	TestTailNode02();
...
void ArpGenericTest1Application::TestNodeAt_Forwards02()
{
	ArpIndexedList	*list = new ArpIndexedList(10);	
	assert(list != NULL);
	ArpIndexedListAccess	access(*list, ArpIndexedListAccess::AccessWrite);
	...
	AddNodeIndex(&access, 20);
	NodeAtShouldBe(0, 20, 0, ARP_FORWARDS, &access);

	access.DeleteNodeContents();
	access.DeleteList();
}

void ArpGenericTest1Application::TestNodeFor01()
{
	ArpIndexedList	*list = new ArpIndexedList(10);	
	assert(list != NULL);
	ArpIndexedListAccess	access(*list, ArpIndexedListAccess::AccessWrite);
	ArpTestNode		*node20, *node9, *node9_2;

	AddNodeIndex(20, &node20, &access);

	NodeForShouldBe(20, node20->RawContents(), node20, &access);
	NodeForShouldBe(21, node20->RawContents(), node20, &access);
	NodeForShouldBe(41, node20->RawContents(), node20, &access);
	NodeForShouldBe(0, node20->RawContents(), node20, &access);

	AddNodeIndex(20, &node9, &access);

	NodeForShouldBe(20, node20->RawContents(), node20, &access);
	NodeForShouldBe(9, node9->RawContents(), node9, &access);
	NodeForShouldBe(10, node20->RawContents(), node20, &access);
	NodeForShouldBe(0, node9->RawContents(), node9, &access);
	NodeForShouldBe(20, node9->RawContents(), node9, &access);
	NodeForShouldBe(9, node20->RawContents(), node20, &access);

	AddNodeIndex(9, &node9_2, &access);

	NodeForShouldBe(9, node9->RawContents(), node9, &access);
	NodeForShouldBe(9, node9_2->RawContents(), node9_2, &access);
	NodeForShouldBe(0, node9->RawContents(), node9, &access);
	NodeForShouldBe(30, node9_2->RawContents(), node9_2, &access);
...
// NOTE:  This test will junk up memory -- it does not delete the contents of any
// of the nodes.
void ArpGenericTest1Application::TestRemoveNode01()
{
	...
	AddNodeIndex(&access, 5);
	CountShouldBe(1, &access);
	DeleteNodeIndex(0, 0, &access);
	CountShouldBe(0, &access);

	AddNodeIndex(&access, 32);
	AddNodeIndex(&access, 1);
	RemoveNodeIndex(3, 0, &node, &access);
	CountShouldBe(1, &access);
	NodeShouldBe(node, 32);
...
void ArpGenericTest1Application::TestTailNode02()
{
	ArpIndexedList	*list = new ArpIndexedList(10);	
	ArpTestNode		*node;
	assert(list != NULL);
	...
	AddNodeIndex(&access, 10);
	AddNodeIndex(&access, 0);
	AddNodeIndex(&access, 29);
	AddNodeIndex(&access, 49);

	TailNodeShouldBe(49, &access);
	RemoveNodeIndex(49, 0, &node, &access);
	TailNodeShouldBe(29, &access);
	node->DeleteListContents();
	delete node;

	RemoveNodeIndex(10, 0, &node, &access);
	TailNodeShouldBe(29, &access);
	node->DeleteListContents();
	delete node;

	RemoveNodeIndex(29, 0, &node, &access);
	TailNodeShouldBe(9, &access);
	node->DeleteListContents();
	delete node;

	RemoveNodeIndex(0, 0, &node, &access);
	TailNodeShouldBe(9, &access);
	node->DeleteListContents();
	delete node;
...
bool ArpGenericTest1Application::NodeForShouldBe(int32 indexToFind, void *contents,
						ArpTestNode *node, ArpIndexedListAccess *access)
{
	ArpTestNode	*foundNode;
	if (access->NodeFor(indexToFind, contents, (ArpIndexedNode**)&foundNode) != B_OK) {
		printf("ERROR  NodeForShouldBe(%li) -- Node should exist\n", indexToFind);
		fflush(stdout);
		assert(false);
		return false;
	}
	if (foundNode == node) return true;

	printf("ERROR  NodeForShouldBe(%li) -- Node should be identical (%li)\n", indexToFind, foundNode->Index());
	fflush(stdout);
	assert(false);
	return false;
}
...
bool ArpGenericTest1Application::AddNodeIndex(int32 index,
				ArpTestNode **addedNode,
				ArpIndexedListAccess *access)
{
	ArpTestNode		*node = NewNode(index);
	if (access->AddNode(node) != B_OK) {
...
bool ArpGenericTest1Application::RemoveNodeIndex(int32 index,
				int32 findNode,
				ArpTestNode **removedNode,
				ArpIndexedListAccess *access)
{
	ArpTestNode		*node;
	if (access->NodeAt(index, (ArpIndexedNode**)&node, findNode) != B_OK) {
		printf("RemoveNodeIndex(%li, %li)  ERROR!  Node should exist %li\n", index, findNode);
		fflush(stdout);
		assert(false);
		return false;
	}
	*removedNode = node;
	return RemoveNode(node, access);
}

bool ArpGenericTest1Application::RemoveNode(ArpTestNode *node,
				ArpIndexedListAccess *access)
{
	if (access->RemoveNode(node) != B_OK) {
		printf("RemoveNode()  ERROR!  Should be allowed to remove node (%li)\n", node->Index());
		fflush(stdout);
		assert(false);
		return false;
	}
...
bool ArpGenericTest1Application::DeleteNodeIndex(int32 index,
				int32 findNode,
				ArpIndexedListAccess *access)
{
	ArpTestNode		*node;
	if (access->NodeAt(index, (ArpIndexedNode**)&node, findNode) != B_OK) {
		printf("DeleteNodeIndex(%li, %li)  ERROR!  Node should exist %li\n", index, findNode);
		fflush(stdout);
		assert(false);
		return false;
	}
	return DeleteNode(node, access);
}

bool ArpGenericTest1Application::DeleteNode(ArpTestNode *node,
				ArpIndexedListAccess *access)
{
	if (access->DeleteNode(node) != B_OK) {
		printf("DeleteNode()  ERROR!  Should be allowed to delete node (%li)\n", node->Index());
		fflush(stdout);
		assert(false);
		return false;
	}
