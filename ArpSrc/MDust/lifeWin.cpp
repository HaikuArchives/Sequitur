	...
	: BWindow(BRect(100,100,510,510), "3D Life", B_TITLED_WINDOW, 0)
{
	...
	AddChild(mv);
#else
	mLearnView = new learnView(BRect(0,20,410,410));
	AddChild(mLearnView);
#endif
...
#else
	menu->SetTargetForItems(mLearnView);
#endif
...
#else
	xmenu->SetTargetForItems(mLearnView);
	ymenu->SetTargetForItems(mLearnView);
	zmenu->SetTargetForItems(mLearnView);
#endif
	mb->AddItem(menu);
	AddChild(mb);
