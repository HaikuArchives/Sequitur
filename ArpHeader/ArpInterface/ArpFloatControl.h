...
#ifndef ARPINTERFACE_ARPFLOATCONTROL_H
...
{
public:
	ArpFloatControl(BRect frame, const char* name,
					const BString16* label, BMessage* message,
	 ...
	 * in the control.
	 */
	void			GetValueLabel(float value, BString16& str) const;
	BRect			ControlBounds() const;
	/* The default message is set with every mouse move; this is
	 * sent on mouse up.
	 */
	...
	/* Do a bounds check, don't send out notification.
	 */
	void			SetValueQuiet(float newValue);
...
protected:
	...
	virtual status_t ValueFromLabel(const BString16* label, float* answer) const;
